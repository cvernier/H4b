...
void MakeFakeData()
{
  std::string filename="../../MMMM_nominal/a_KinFit/Histograms_DiJetPt_glugluToX300ToHHTobbbb_8TeV_width1MeV_GEN_FASTSIM_HLT.root";
  int rebin=1;
  std::string filename_old = "/gpfs/ddn/cms/user/cvernier/H4b_step2/CMSSW_5_3_3_patch2/src/VHbbAnalysis/VHbbDataFormats/bin/DiJetPt_glugluToX300ToHHTobbbb_8TeV_width1MeV_GEN_FASTSIM_HLT.root";
...
//std::cout<<"h_mX_SR->GetSumOfWeights() = "<<h_mX_SR->GetSumOfWeights()<<std::endl;
//std::cout<<"h_mX_SR_fakeData->GetSumOfWeights() = "<<h_mX_SR_fakeData->GetSumOfWeights()<<std::endl;
...
// std::cout<< h_mX_SR_signal->GetSumOfWeights()*prodXsec_1*totalLumi/nSignal_init<<" asd "<<std::endl; 
// std::cout<< " asd "<<  h_mX_SR_fakeData->GetSumOfWeights() <<std::endl;
  ...
  w->import(data_obs);
  std::string mass="300";//filename.substr(27,3);
...
std::cout<<"1 pb yield = "<<h_mX_SR_fakeData->Integral(h_mX_SR_fakeData->FindBin(SR_lo), h_mX_SR_fakeData->FindBin(SR_hi)-1)<<std::endl;
