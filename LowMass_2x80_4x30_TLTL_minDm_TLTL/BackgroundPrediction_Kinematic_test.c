...
void fillByThrowTH1(TH1 *h_in, TH1 *h_out, double weight=1.)
{
  ...
  int throws=int(new_max_h_in*50.);//50
...
void BackgroundPrediction_Kinematic_test()
{
...
//RooRealVar x("x", "m_{X} (GeV)", VR_lo, VR_hi);
  ...
  RooRealVar bC_p4("bC_p4", "bC_p4", 250., 350.);
    sg_p0=new RooRealVar("sg_p0", "sg_p0", 0., 1.);
      ...
      sg_p3=new RooRealVar("sg_p3", "sg_p3", 0., 12.);
...
sg_p6=new RooRealVar("sg_p6", "sg_p6", 0., 5.);
  ...
  polyn.fitTo(bC_data, RooFit::Range(VR_lo, VR_hi));
  RooPlot *bC_plot=x.frame();
  bC_data.plotOn(bC_plot, RooFit::MarkerColor(kRed));
  polyn.plotOn(bC_plot, RooFit::LineColor(kRed));
...
*/
  ...
  sg_3=new RooRealVar("sg_3", "sg_3", 0., 12.);
 ...
 sg_6=new RooRealVar("sg_6", "sg_6", 0., 5.);
  ...
  //RooDataHist bS_data("bS_data", "bS Data", RooArgList(x), h_mMMMMb_3Tag_SR);
  polyn_.fitTo(bS_data, RooFit::Range(VR_lo, VR_hi));
  RooPlot *bS_plot=x.frame();
  bS_data.plotOn(bS_plot, RooFit::MarkerColor(kBlue));
  polyn_.plotOn(bS_plot, RooFit::LineColor(kBlue));
  std::cout<<" === === "<<std::endl;
...
*/  c_bC->SaveAs(("c_compareData_"+tags+"_VR_RooFit_GaussExp.png").c_str());
  ...
  double PbC_0=sg_p0->getVal();
  double PbC_1=sg_p1->getVal();
  double PbC_2=sg_p2->getVal();
  double PbC_3=sg_p3->getVal();
  double PbC_4=sg_p4->getVal();
  double PbC_5=sg_p5->getVal();
  double dPbC_0=sg_p0->getError();
  double dPbC_1=sg_p1->getError();
  double dPbC_2=sg_p2->getError();
  double dPbC_3=sg_p3->getError();
  double dPbC_4=sg_p4->getError();
  double dPbC_5=sg_p5->getError();  


  double PbS_0=sg_0->getVal();
  double PbS_1=sg_1->getVal();
  double PbS_2=sg_2->getVal();
  double PbS_3=sg_3->getVal();
  double PbS_4=sg_4->getVal();
  double PbS_5=sg_5->getVal();
  double dPbS_0=sg_0->getError();
  double dPbS_1=sg_1->getError();
  double dPbS_2=sg_2->getError();
  double dPbS_3=sg_3->getError();
  double dPbS_4=sg_4->getError(); 
  double dPbS_5=sg_5->getError();
  ...
  RooRealVar *x, *bg_p0, *bg_p1, *bg_p2, *bg_p3, *bg_p4, *bg_p5, *bg_p6;
...
bg_p6=new RooRealVar("bg_p6", "bg_p6", 0., 5.);
  ...
  RooBernstein bg("bg", "bg", *x, RooArgList(*bg_p1,*bg_p2, *bg_p3, *bg_p4));//, *bg_p5));// , *bg_p6));
...
//  LowMassBackgroundPDF bg("bg", "bg", x, bg_p0, bg_p1, bg_p2, bg_p3, bg_p4);
  ...
  bg.fitTo(pred, RooFit::Range(SR_lo, 600));
  ...
  */
  ...
  double PaS_5=PaC_5*PbS_5/PbC_5;
  ...
  double dPaS_5=PaS_5*quad((dPaC_5/PaC_5), (dPbS_5/PbS_5), (dPbC_5/PbC_5));
  ...
  std::cout<<"(dPaC_4/PaC_4) = ("<<dPaC_4<<"/"<<PaC_4<<") = "<<(dPaC_4/PaC_4)<<"; (dPbS_4/PbS_4) = ("<<dPbS_4<<"/"<<PbS_4<<") = "<<(dPbS_4/PbS_4)<<"; (dPbC_4/PbC_4) = ("<<dPbC_4<<"/"<<PbC_4<<") = "<<(dPbC_4/PbC_4)<<std::endl;
   std::cout<<"(dPaC_5/PaC_5) = ("<<dPaC_5<<"/"<<PaC_5<<") = "<<(dPaC_5/PaC_5)<<"; (dPbS_5/PbS_5) = ("<<dPbS_5<<"/"<<PbS_5<<") = "<<(dPbS_5/PbS_5)<<"; (dPbC_5/PbC_5) = ("<<dPbC_5<<"/"<<PbC_5<<") = "<<(dPbC_5/PbC_5)<<std::endl;
  ...
  std::cout<<" Predicted PaS_5 = "<<PaS_5<<" +- "<<dPaS_5<<std::endl;
  RooRealVar *bg_pred0;
  RooRealVar *bg_pred1;
  RooRealVar *bg_pred2;
  RooRealVar *bg_pred3;
  ...
  RooRealVar *bg_pred5;
   //RooRealVar x("x", "m_{X} (GeV)", SR_lo, SR_hi);
  //RooBernstein bg_pred("bg_pred", "Background Predicted for Signal Region", *x, RooArgList(*bg_pred0,*bg_pred1, *bg_pred2, *bg_pred3,*bg_pred4, *bg_pred5)); 
  //RooBernstein bg_pred("bg_pred", "Background Predicted for Signal Region", *x, RooArgList(*bg_pred0,*bg_pred1, *bg_pred2, *bg_pred3,*bg_pred4, *bg_pred5));
//LowMassBackgroundPDF bg_pred("bg_pred", "Background Predicted for Signal Region", x, bg_pred0, bg_pred1, bg_pred2, bg_pred3, bg_pred4);
  ...
  else
  {
    ...
    bg_pred5=new RooRealVar("bg_pred5", "bg_pred5", PaS_5);  bg_pred5->setError(dPaS_5);
  }
 // LowMassBackgroundPDF bg_pred("bg_pred", "Background Predicted for Signal Region", x, bg_pred0, bg_pred1, bg_pred2, bg_pred3, bg_pred4);
  RooBernstein bg_pred("bg_pred", "Background Predicted for Signal Region", *x, RooArgList(*bg_pred0,*bg_pred1, *bg_pred2, *bg_pred3));//,*bg_pred4));//, *bg_pred5));
  ...
  if (tags!="MMMM")
  {
    ...
    bg_pred.fitTo(unblind, RooFit::Range(SR_lo, 600));
  ...
  {
    bg_pred.plotOn(aC_plot, RooFit::LineColor(kBlue), RooFit::Range(SR_lo, 600));
    aC_plot->Draw("sames");
...
// ***** fit fake data with same function ***** /
  ...
  bg.fitTo(unblind, RooFit::Range(SR_lo, 600));
...
//for(int n =0; n<5; n++){ 
  TH1F *h_mX_SR_signal=(TH1F*)signal->Get("h_mX_SR"); 
  //h_mX_SR_signal->Scale((prodXsec_1[n]*totalLumi/nSignal_init)); 
/*
  RooRealVar *sg_p0, *sg_p1;
  {
    if (point=="300")
    {
      sg_p0=new RooRealVar("sg_p0", "sg_p0", 290., 315.);
      sg_p1=new RooRealVar("sg_p1", "sg_p1", 0., 20.);
    }
    else if (point=="400")
    {
  ...
  RooGaussian signal_("signal_", "Signal Prediction", *x, *sg_p0, *sg_p1);
  RooDataHist signalHistogram("signalHistogram", "Signal Histogram", RooArgList(*x), h_mX_SR_signal);
  signal_.fitTo(signalHistogram, RooFit::Range(SR_lo, SR_hi), RooFit::Save());
*/ 
RooRealVar *sg_p0, *sg_p1, *sg_p2, *sg_p3, *sg_p4, *sg_p5, *p0, *p1, *alpha, *nne, *sg_p6;
{
    if (point=="300")
    {
      p0=new RooRealVar("p0", "p0", 290., 315.);
      p1=new RooRealVar("p1", "p1", 500., 615.);
      p0->setVal(250);
      p1->setVal(600); 
      sg_p0=new RooRealVar("sg_p0", "sg_p0", 295., 315.);
      sg_p1=new RooRealVar("sg_p1", "sg_p1", 3., 10.);
      sg_p2=new RooRealVar("sg_p2", "sg_p2", 0., 10.);
      sg_p3=new RooRealVar("sg_p3", "sg_p3", 0., 10.);
      sg_p4=new RooRealVar("sg_p4", "sg_p4", 0.,10.);
      sg_p5=new RooRealVar("sg_p5", "sg_p5",0., 1.);
      sg_p6=new RooRealVar("sg_p6", "sg_p6", 0., 10.);
    }
    else if (point=="400")
    {
      p0=new RooRealVar("p0", "p0", 290., 315.);
      ...
      p0->setVal(240);
      p1->setVal(500);
      sg_p0=new RooRealVar("sg_p0", "sg_p0", 400., 410.);
      sg_p1=new RooRealVar("sg_p1", "sg_p1", 5., 10.);
    //  sg_p2=new RooRealVar("sg_p2", "sg_p2", 0., 12.);
    //  sg_p3=new RooRealVar("sg_p3", "sg_p3", -10., 0.);
    //  sg_p4=new RooRealVar("sg_p4", "sg_p4", 0., 12.);
      sg_p2=new RooRealVar("sg_p2", "sg_p2", 0., 10.);
      sg_p3=new RooRealVar("sg_p3", "sg_p3", 0., 10.);
      sg_p4=new RooRealVar("sg_p4", "sg_p4", 0., 10.);
      sg_p6=new RooRealVar("sg_p6", "sg_p6", 0., 10.);
  ...
  //RooChebychev Cpolyn("Cpolyn", "Combinatoric", *x, RooArgList(*sg_p2, *sg_p3, *sg_p4));
  //WrappedChebychev3 Cpolyn("Cpolyn", "Combinatoric", *x, *p0, *p1,*sg_p2, *sg_p3, *sg_p4);
  RooGaussian signalCore("signalCore", "Signal Prediction", *x, *sg_p0, *sg_p1);

    //  m=new RooRealVar("m", "m", 100., 150.);
    // m0=new RooRealVar("m0", "m0", 115., 135.);
     nne=new RooRealVar("nne", "nne", 0., 10.);
     alpha=new RooRealVar("alpha", "alpha", 0., 10.);
    // sigma=new RooRealVar("sigma", "sigma", 0., 30.);

  //RooGaussian signal("signal", "Signal Prediction", *x, *sg_p0, *sg_p1);
//  RooCBShape signalCore("signalCore", "Signal Prediction", *x, *sg_p0, *sg_p1,*alpha, *nne);
   RooBernstein Cpolyn("Cpolyn", "Combinatoric", *x, RooArgList(*sg_p2, *sg_p3, *sg_p4)); 
//RooChebychev Cpolyn("Cpolyn", "Combinatoric", *x, RooArgList(*sg_p2, *sg_p3, *sg_p4));

  RooAddPdf signal_("signal_", "signal", RooArgList(signalCore, Cpolyn), *sg_p5);
  RooDataHist signalHistogram("signalHistogram", "Signal Histogram", RooArgList(*x), h_mX_SR_signal);
  signal_.fitTo(signalHistogram, RooFit::Range(250.,600.), RooFit::Save(),RooFit::SumW2Error(kFALSE)); 
 //polyn.fitTo(signalHistogram, RooFit::Range(rangeLo, rangeHi), RooFit::Save());

   RooPlot *plot=x->frame();
  signalHistogram.plotOn(plot, RooFit::MarkerColor(kBlue));
  signal_.plotOn(plot, RooFit::LineColor(kBlue), RooFit::LineWidth(0));
...
/*
RooRealVar *sg_p00, *sg_p01, *sg_p02, *sg_p03, *sg_p04, *sg_p05, *p00, *p01, *alpha0, *nne0, *sg0_p6;   
//x=new RooRealVar("x", "m_{X} (GeV)", SR_lo, SR_hi);
//x->setBins(35);
sg_p00=new RooRealVar("sg_p00", "sg_p00", sg_p0->getVal());  sg_p00->setError(sg_p0->getError());
sg_p01=new RooRealVar("sg_p01", "sg_p01", sg_p1->getVal());  sg_p01->setError(sg_p1->getError());
sg_p02=new RooRealVar("sg_p02", "sg_p02", sg_p2->getVal());  sg_p02->setError(sg_p2->getError());
sg_p03=new RooRealVar("sg_p03", "sg_p03", sg_p3->getVal());  sg_p03->setError(sg_p3->getError());
sg_p04=new RooRealVar("sg_p04", "sg_p04", sg_p4->getVal());  sg_p04->setError(sg_p4->getError());
sg_p05=new RooRealVar("sg_p05", "sg_p05", sg_p5->getVal());  sg_p05->setError(sg_p5->getError());
alpha0=new RooRealVar("alpha0", "alpha0", alpha->getVal());  alpha0->setError(alpha->getError());
nne0=new RooRealVar("nne0", "nne0", nne->getVal());  nne0->setError(nne->getError());
p00=new RooRealVar("p00", "p00", 1., 1.3);

sg_p00->setConstant(kTRUE);
sg_p01->setConstant(kTRUE);
sg_p02->setConstant(kTRUE);
sg_p03->setConstant(kTRUE);
sg_p04->setConstant(kTRUE);
sg_p05->setConstant(kTRUE);
alpha0->setConstant(kTRUE);
nne0->setConstant(kTRUE);
//p00->setConstant(kTRUE);
RooFormulaVar xx("xx","xx","x*p00",RooArgList(*p00,*x));

 RooCBShape signalCore_fix("signalCore_fix", "Signal Prediction fixed", xx, *sg_p00, *sg_p01,*alpha0, *nne0);
   RooBernstein Cpolyn_fix("Cpolyn", "Combinatoric", *x, RooArgList(*sg_p02, *sg_p03, *sg_p04));

  RooAddPdf signal_fix("signal_fix", "signal", RooArgList(signalCore_fix, Cpolyn_fix), *sg_p05);
  RooDataHist signalHistogram_fix("signalHistogram_fix", "Signal Histogram", RooArgList(*x), h_mX_SR_signal);
  signal_fix.fitTo(signalHistogram_fix, RooFit::Range(255.,500.), RooFit::Save(),RooFit::SumW2Error(kFALSE));
 //polyn.fitTo(signalHistogram, RooFit::Range(rangeLo, rangeHi), RooFit::Save());

   RooPlot *plot=x->frame();
  signalHistogram_fix.plotOn(plot, RooFit::MarkerColor(kBlue));
  signal_fix.plotOn(plot, RooFit::LineColor(kBlue), RooFit::LineWidth(0));
   TCanvas *c_data1=new TCanvas("c_data1", "c_data1", 500, 500);
...
*/
  ...
  std::cout<< "  n  "<<prodXsec_1[n]*totalLumi/nSignal_init<<std::endl;
  std::cout<<h_mX_SR_signal->GetIntegral()<<"  asd  "<<std::endl;
  ntoys_signal = h_mX_SR_signal->Integral(h_mX_SR_signal->FindBin(SR_lo),h_mX_SR_signal->FindBin(SR_hi)-1)*prodXsec_1[n]*totalLumi/nSignal_init;
  RooDataHist * data_sig= signal_.generateBinned(*x,ntoys_signal, RooAbsData::Poisson);
  RooDataHist* data_ = bg.generateBinned(*x,num_toys, RooAbsData::Poisson);

// RooPlot *plot=x->frame();
//data_->plotOn(plot);
//plot->Draw("");
   //TH1 * h_fake_noSig = data_->createHistogram("h_fake_noSig",*x);
   //h_fake_noSig->Add(h_mX_SR_signal);
   if(n!=0) data_->add(*data_sig);
  // RooDataHist data_obs(*data_, "data_obs");
   TH1 * h_fake = data_->createHistogram("h_fake",*x);
   //h_fake_noSig->Draw(); 
   //h_fake->Draw("");
   TH1F *h_fakeData=(TH1F*)h_mX_SR_signal->Clone("h_fakeData");
   h_fakeData->Reset();
  ...
  ss << prodXsec_1[n];//add number to the stream
  std::cout<<"mass = "<<point<<std::endl;
  w1->SaveAs(("w_data_"+point+"_"+ss.str()+".root").c_str());
  c_data->Print(("c_data_"+point+"_"+ss.str()+".png").c_str());
  }
